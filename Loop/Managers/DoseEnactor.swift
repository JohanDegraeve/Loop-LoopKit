    ...
    func enact(recommendation: AutomaticDoseRecommendation, with pumpManager: PumpManager, completion: @escaping (PumpManagerError?) -> Void) {
        ...
        dosingQueue.async {
            ...
            if let basalAdjustment = recommendation.basalAdjustment {
                ...
                pumpManager.enactTempBasal(unitsPerHour: basalAdjustment.unitsPerHour, for: basalAdjustment.duration, automatic: basalAdjustment.automatic, manuallyEntered: basalAdjustment.manuallyEntered, completion: { error in
                    if let error = error {
